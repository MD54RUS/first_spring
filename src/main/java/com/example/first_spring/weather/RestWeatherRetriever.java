...
public class RestWeatherRetriever implements WeatherRetriever {
    ...
    // java.net.URI.create("https://community-open-weather-map.p.rapidapi.com/weather");
    ...
    @Override
    public ForecastImportDTO getForecast(String city) {
        ...
        ResponseEntity<ForecastImportDTO> result =
                restTemplate.getForEntity(
                        API_URL + GetApiKey.getKey() + "&q=" + city + "&days=1", ForecastImportDTO.class);
        // todo NPEx check
        logger.debug("Получен прогноз для города{}", result.getBody());
